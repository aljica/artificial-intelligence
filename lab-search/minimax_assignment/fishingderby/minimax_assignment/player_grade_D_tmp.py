...
class Minimax:
    ...
    def alphabeta(self, node, depth, alpha, beta, start_time):
        ...
        if depth == 0 or time.time() - start_time > 0.043:
            v = self.h(node)
    ...
    def alpha_beta_move_old(self, node, depth, alpha, beta, start_time):
        player = node.state.get_player()
        children = node.compute_and_get_children()

        if depth == 0 or (time.time() - start_time > 0.05):
            v = self.h(node)
    ...
    def find_best_move_old(self, root):
        max_depth = 6
        children = root.compute_and_get_children()
        ...
        #print("best move", ACTION_TO_STR[bestChild.move])
