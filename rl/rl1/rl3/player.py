...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        # Greedy choice of action (action with highest Q-value) or random choice of action?
        r = np.random.random()
        if r < epsilon:
        ...
        # It is recommended you use the np.random module
        sched_linear = ScheduleLinear(anneal_timesteps, epsilon_final, epsilon_initial)
        epsilon = sched_linear.value(current_total_steps)
        r = np.random.random()
        if r < epsilon:
            # Choose action randomly
            action = np.random.choice(all_actions)
